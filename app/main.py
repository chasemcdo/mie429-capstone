...
async def label(cache_id: str, file: UploadFile):
    ...
    tip.cache = {
        ...
        "adapter_weights": tensor(redisai_client.tensorget(f"{cache_id}-adapter_weights")),
...
async def generate_cache(labels: List[str], files: List[UploadFile]):
    ...
    redisai_client.tensorset(f"{cache_id}-adapter_weights", tip.cache["adapter_weights"].cpu().numpy())
